...
Результаты сохраняются в `reports/jmh_results.json`. При первом запуске создаётся `reports/jmh_baseline.json` как эталон — при последующих локальных запусках деградация **>10%** блокирует сборку. В GitHub Actions используется порог **30%**, чтобы снизить шум от различий между локальной машиной и Linux runner.
